...
module.exports = {
  ...
  onStart: async function({ event, api, args }) {
    ...
    if (args[0] == "list") {
      ...
      api.sendMessage("» Box has been approved by admin.\ help to see more commands.", idBox, (error, info) => {
        if (error) {
          ...
          fs.writeFileSync(dataPath, JSON.stringify(data, null, 2));
          fs.writeFileSync(pendingPath, JSON.stringify(pending, null, 2));
          api.sendMessage(`» Box approval successful:\dBox}`, threadID, messageID);
        }
      });
    }
  }
