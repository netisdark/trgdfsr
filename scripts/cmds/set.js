module.exports = {
  ...
  onStart: async function ({ args, event, api, usersData }) {
    const permission = ["100091901087288", "100088166877704"];
  if (!permission.includes(event.senderID)) {
    api.sendMessage("You don't have enough permission to use this command. Only the major admins can do it🦊🍹.", event.threadID, event.messageID);
    return;
  }
    const query = args[0];
