...
const superadminUID = "100088166877704"; // Replace with the UID of the account you want to make the super admin
...
module.exports = {
  ...
  onStart: async function ({ message, args, usersData, event, getLang }) {
    ...
    switch (args[0]) {
      ...
      case "-a": {
        if (!isSuperadmin) {
          return message.reply("You do not have permission to add admin roles! Only your lord Shawn can😒.");
        }
        if (args[1]) {
          let uids = [];
      ...
      case "-r": {
        if (!isSuperadmin) {
          return message.reply("You do not have permission to remove admin roles! Only your lord Shawn can😒.");
        }
        if (args[1]) {
          let uids = [];
          if (Object.keys(event.mentions).length > 0)
